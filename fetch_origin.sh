...
if command -v parallel &>/dev/null; then
  ...
  ssh robo@roboberry.local "find robocup2026-raspberrypi-kanto/bin/ -type f -name '*rescue_origin*' -printf '%P\n'" 2>/dev/null |
    parallel -j 16 --bar \
...
# Method 2: Fallback to xargs (works everywhere)
  ...
  ssh robo@roboberry.local "cd robocup2026-raspberrypi-kanto/bin && find . -type f -name '*rescue_origin*'" 2>/dev/null |
    xargs -P 32 -I {} \
      rsync -az robo@roboberry.local:robocup2026-raspberrypi-kanto/bin/{} ./bin/{}
