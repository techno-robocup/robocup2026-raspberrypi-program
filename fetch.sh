...
if command -v parallel &>/dev/null; then
  ...
  ssh robo@roboberry.local "find robocup2026-raspberrypi-program/bin/ -type f -printf '%P\n'" 2>/dev/null |
    parallel -j 16 --bar \
...
# Method 2: Fallback to xargs (works everywhere)
  ...
  ssh robo@roboberry.local "cd robocup2026-raspberrypi-program/bin && find . -type f" 2>/dev/null |
    xargs -P 32 -I {} \
      rsync -az robo@roboberry.local:robocup2026-raspberrypi-program/bin/{} ./bin/{}
